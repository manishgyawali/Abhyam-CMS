...
const Herotestinomialssection = () => {
  ...
  const Data = [
    {
        name: "Your Global ",
        position: "Guiding you through d beyond",
      description:"uiding you through d beyuiding you through d bey",
  ...
  const handleSubmit = (values, { setSubmitting, resetForm }) => {
    ...
    request
      .then(() => {
        toast.success(isEditMode ? "Herotestinomialssection updated!" : "Herotestinomialssection submitted!");
        setIsEditMode(false);
        setEditIndex(null);
        resetForm();
      })
      .catch(() => {
        toast.error(isEditMode ? "Failed to update !" : "Failed to submit !");
      })
        ...
        {({ setFieldValue, isSubmitting }) => (
          <Form>
            <div className="py-10 px-6 flex flex-col gap-6 bg-[#F9FAFB] rounded-2xl">
              <h1 className="text-2xl font-semibold">{isEditMode ? 'Edit Herotestinomialssection' : 'Add Herotestinomialssection'}</h1>
            ...
            <tbody>
              {Data.map((val, index) => (
                <tr key={index} className="border-b">
                  <td className="p-4">
                    <img src={val.image} alt={val.name} className="h-20 w-20 rounded object-cover" />
                  </td>
                  <td className="p-4">{val.name}</td>
                  <td className="p-4">{val.position}</td>
...
export default Herotestinomialssection;